...
void main() {
  test('Given an empty string with no U+001B character, '
      'when stripAnsi is called, '
      'then the output is still empty.', () {
    expect(stripAnsi(''), '');
  });

  test('Given plain ASCII text with no U+001B character, '
      'when stripAnsi is called, '
      'then the output equals the input.', () {
    expect(stripAnsi('plain'), 'plain');
  });

  test('Given text that contains bracket notation resembling CSI but no ESC, '
      'when stripAnsi is called, '
  ...
  test('Given a Windows-style path string with backslashes and brackets, '
      'when stripAnsi is called, '
      'then the output equals the input unchanged.', () {
  ...
  test('Given Dart source text with generics and list literals, '
      'when stripAnsi is called, '
      'then the output equals the input unchanged.', () {
  ...
  test('Given an analyzer-style file URI error line without ESC, '
      'when stripAnsi is called, '
      'then the output equals the input unchanged.', () {
  ...
  test('Given a version string with bracketed suffix, '
      'when stripAnsi is called, '
      'then the output equals the input unchanged.', () {
  ...
  test('Given nested square brackets in user text, '
      'when stripAnsi is called, '
      'then the output equals the input unchanged.', () {
  ...
  test('Given source-span output with CSI color and emphasis codes, '
      'when stripAnsi is called, '
  ...
  test('Given two separately styled words with CSI around each run, '
      'when stripAnsi is called, '
  ...
  test('Given extended CSI parameters such as 256-color foreground, '
      'when stripAnsi is called, '
      'then the full CSI sequence is removed.', () {
    expect(stripAnsi('\x1b[38;5;12mcolor\x1b[0m'), 'color');
  });

  test('Given box-drawing diagnostic lines with CSI around glyphs, '
      'when stripAnsi is called, '
  ...
  test('Given Unicode text with CSI around a symbol in the middle, '
      'when stripAnsi is called, '
  ...
  test('Given a string that no longer contains CSI sequences, '
      'when stripAnsi is called, '
      'then the output equals the input.', () {
  ...
  test('Given a string produced by stripping ANSI once, '
  ...
  test('Given text that ends with a lone ESC byte and no CSI introducer, '
      'when stripAnsi is called, '
      'then the lone ESC is preserved.', () {
    expect(stripAnsi('ok\x1b'), 'ok\x1b');
  });

  test('Given ESC followed by a character other than [, '
      'when stripAnsi is called, '
